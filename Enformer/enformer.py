...
# SEQUENCE_LENGTH = 115_200 # works with attention!
BIN_SIZE = 128
TARGET_LENGTH = 896
...
#         conv_block(channels * 2, 1),
        conv_block(channels // 8, 1), # REDUCING FILTER SPACE
    ...
    classifier_name_scope = tf.name_scope('classifier')
    classifier_name_scope.__enter__()
    
    self._emb_emb = Sequential(lambda: [snt.Linear(128),
                                        snt.Dropout(0.5),
                                        tf.nn.relu], name='emb_emb')
    
    self._seq_emb = Sequential(lambda: [snt.Linear(512),
#                                         snt.BatchNorm(create_scale=True,
#                                                       create_offset=True,
#                                                       decay_rate=0.9,
#                                                       scale_init=snt.initializers.Ones()),
                                        tf.nn.softplus], name='seq_emb')
    
    self._final_fc = Sequential(lambda: [
                                  snt.Linear(256),
                                  tf.nn.relu,
#                                   snt.BatchNorm(create_scale=True,
#                                     create_offset=True,
#                                     decay_rate=0.9,
#                                     scale_init=snt.initializers.Ones()),
                                  snt.Linear(1)], name='final_fc')
    
    
    classifier_name_scope.__exit__(None, None, None)
  ...
  @property
  def emb_emb(self):
    return self._emb_emb

  @property
  def seq_emb(self):
    return self._seq_emb

  @property
  def final_fc(self):
    return self._final_fc

  def __call__(self, inputs: tf.Tensor, embeddings: tf.Tensor,
               is_training: bool) -> Dict[str, tf.Tensor]:
        
    trunk_embedding = self.trunk(inputs, is_training=is_training)
    trunk_embedding = tf.reshape(trunk_embedding, [1, 1, -1]) # FLATTENING TO SINGLE PREDICTION
    sequence_embedding = self.seq_emb(trunk_embedding, is_training=is_training)

#     embeddings = tf.expand_dims(embeddings, axis=1)
    rep_seq = tf.repeat(sequence_embedding, tf.shape(embeddings)[0], axis=0)

    drop_embeddings = self.emb_emb(embeddings, is_training=is_training)
    final_embedding = tf.concat([rep_seq, drop_embeddings], axis=2)

    return self.final_fc(final_embedding, is_training=is_training)
...
class TargetLengthCrop1D(snt.Module):
  ...
  #@tf.autograph.experimental.do_not_convert # SUPPRESSING ERRORS 
