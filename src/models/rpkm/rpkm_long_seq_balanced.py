...
def bn_relu_conv(in_channels, out_channels, k, d):
    stack = nn.Sequential(
        ...
        nn.Conv1d(in_channels, out_channels, kernel_size = k, dilation = d, padding = get_padding(k, d)),
    )
    return stack
...
class DeepCT(nn.Module):
    ...
    def pick_embeddings(self, embeddings, cell_targets):
        ...
        tiled_embeddings = embeddings.repeat(cell_targets.size()[0], 1) # repeat all available embeddings batch_size times
        flat_targets = cell_targets.flatten()
        out_embeddings = tiled_embeddings[flat_targets==1, :] # select embeddings to use for this mini-batch
