...
from src.criterion import WeightedMSELossWithMPI
import os
import pyBigWig
...
class TestWeightedMSELoss:
    def test_loss(self):
        ...
        loss_manual = np.mean(
            [w * ((x - y) ** 2) for x, y, w in zip(sample_x, sample_y, weights) if w != 0]
        )
        with torch.no_grad():
            loss = l.forward(
...
class TestWeightedMSELossWithMPI:
    def setup_method(self, test_method):
        ...
        self.n_cell_types = 4
        self.n_features = 2
        ct_by_features = np.ones(self.n_cell_types*self.n_features).\
                            reshape(self.n_cell_types,self.n_features)
        ...
        for cell_type in range(self.n_cell_types):
            for feature in range(self.n_features):
                if not self.ct_by_features[cell_type,feature]:
                    continue
                fname = os.path.join(TEST_DATA_PATH, 
                                    "testqDataset" + str(cell_type) + "_" +str(feature)+".bw")
                distinct_features_name = "|".join(map(str,[cell_type,feature,"None"]))
                ...
                bw.addEntries(
                    ...
                    [0, 0],
                    ends=[20000, 20000],
                    values=[float((feature + 1)*100 +  cell_type - 2), 
    ...
    def test_loss(self):
                ...
                for feature in range(self.n_features):
                    for cell_type in range(self.n_cell_types):
                        if self.ct_by_features[cell_type,feature] != 0:
                            means[feature].append(target[batch_item][cell_type,feature])
                means = [np.mean(i) for i in means]
                deviations = np.array(target[batch_item])
                for feature in range(self.n_features):
                    for cell_type in range(self.n_cell_types):
                        deviations[cell_type,feature] = deviations[cell_type,feature] /\
                                                         means[feature]
                predicted_mean, predicted_deviation = input[batch_item,-1,:],\
                                                        input[batch_item,:-1,:] 
                loss_mean = np.mean([(means[i]-predicted_mean[i])**2 for i in range(len(means))])
                loss_dev = np.mean([(deviations[i,j]-predicted_deviation[i,j])**2 \
        ...
        predicts = np.random.normal(100,10,size=(self.batch_size, 
                                                self.n_cell_types + 1, 
                                                self.n_features))
        ...
        for alpha in [1,0.5,0]:
            ...
            assert abs(manual_loss(alpha, predicts, targets) - loss.forward(torch.tensor(predicts),
                                                                        targets)) < CLOSE
