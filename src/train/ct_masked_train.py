...
class TrainEncodeDatasetModel(object):
    ...
    def __init__(
        ...
        self.n_cell_types = (
            n_cell_types  # self.cfg["dataset"]["dataset_args"]["n_cell_types"]
        )
    ...
    def run_masked_train(self):
        ...
        if self.checkpoint_resume is not None:
            ...
            if (
                ...
                checkpoint_basename = "checkpoint-{0}".format(strftime("%m%d%H%M%S"))
            ...
            logger.debug("Saving checkpoint `{0}.pth.tar`".format(checkpoint_basename))
    ...
    def _evaluate_on_data(self, data_loader):
        ...
        for batch in tqdm(data_loader):
            ...
            with torch.no_grad():
                ...
                if self.masked_targets:
                    target_mask = self.target_mask_val.view(
                        -1, self.target_mask_val.shape[-1]
                    )
                if self.val_reduction_factor > 1:
                    reduced_val_batch_size = (
                        predictions.shape[0] // self.val_reduction_factor
